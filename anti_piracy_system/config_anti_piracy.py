...
SUPPORTED_PLATFORMS = {
    "xiaohongshu": {
        ...
        "search_keywords": ["众合法考", "众合法考客观题", "众合法考学习包"]
    },
    "xianyu": {
        "name": "闲鱼",
...
""",
    ...
    # 从列表页批量提取商品信息
    "extract_from_list": """
你现在在小红书商品搜索结果列表页面。请识别屏幕上所有商品卡片。

【商品卡片结构 - 从上到下】
1. 商品图片（占据卡片大部分）
2. 商品标题（1-2行文字）
3. 价格（红色，¥开头）
4. 店铺名称（最底部小字，可能有小头像图标）

【店铺名称 - 最关键】
- 位置：每张卡片的最底部
- 特征：灰色小字，可能有小圆形店铺图标
- 示例："方圆众合教育"、"XX旗舰店"、"小明的店"

请按JSON数组格式返回（只返回JSON）：
...
重要：shop_name 必须从每张卡片底部提取，这是判断是否盗版的关键！
""",
...
""",
    ...
    # 提取商品信息
    "extract_info": """
你现在在小红书商品详情页面。请仔细识别并提取以下信息：

【最重要 - 店铺名称识别】
在小红书商品详情页中，店铺名称的具体位置：
- 位于商品图片下方、商品标题附近
- 通常有一个小圆形头像，头像右边的文字就是店铺名/卖家昵称
- 可能显示为"XXX的店"、"XXX旗舰店"或纯账号名
- 在"进店"按钮旁边的名称
- 如果看到"方圆众合教育"这类名称，这就是店铺名

请严格按以下JSON格式返回（只返回JSON，不要任何其他文字）：

```json
{{
  "shop_name": "店铺名称（头像旁边的文字）",
  "title": "商品标题",
  "price": 898.0,
  "description": "商品描述"
}}
...
""",
    ...
    # 专门提取店铺名称（备用方案）
    "extract_shop_name_only": """
请只回答一个问题：当前页面上显示的店铺名称/卖家昵称是什么？
