...
""",
    ...
    # 从列表页批量提取商品信息
    "extract_from_list": """
当前在小红书商品列表页面，请识别当前屏幕上所有可见的商品卡片信息。

**【商品卡片布局说明】**
每个商品卡片从上到下通常包含：
1. 商品图片（占据卡片上半部分）
2. 商品标题（图片下方的文字）
3. 价格（通常显示为 ¥XX.XX 格式，红色或醒目颜色）
4. 店铺名称（在价格下方或旁边，通常是最底部的小字）

**【店铺名称识别要点】**
- 店铺名称通常在每个卡片的最底部
- 可能显示为"店铺名"、"卖家昵称"或简单的账号名
- 字体通常比标题和价格小
- 可能有店铺图标/头像在旁边

**请按以下JSON数组格式返回（仅返回JSON）：**
```json
[
  {{
    "index": 0,
...
**注意：** 店铺名称是判断盗版的关键信息，请务必仔细识别每个卡片底部的店铺信息。
""",
...
""",
    ...
    # 提取商品信息
    "extract_info": """
当前在小红书商品详情页，请仔细观察整个页面并提取商品信息。

**【重要】店铺名称的位置说明：**
在小红书商品详情页中，店铺/卖家信息的位置通常是：
1. 商品标题的正下方，显示为"头像 + 昵称"的形式
2. 或者在价格附近，有"进店"、"店铺"等入口旁边的名称
3. 可能显示为：店铺名、卖家昵称、商家名称、账号名等
4. 在商品卡片视图中，店铺名称通常在价格下方

**请提取以下信息：**
1. **shop_name** - 店铺名称/卖家昵称（最重要！仔细查找页面上的账号/店铺信息）
2. **title** - 商品标题
3. **price** - 商品价格（数字，不含¥符号）
4. **description** - 商品描述

**返回格式（仅返回JSON，不要其他文字）：**
```json
{{
  "shop_name": "识别到的店铺或卖家名称",
  "title": "商品标题",
  "price": 99.0,
  "description": "商品描述内容"
}}
...
**注意：**
- 如果看到头像旁边的文字，那通常就是店铺/卖家名称
- 店铺名称可能是个人昵称（如"小明的店"）或品牌店铺名
- 如果实在无法识别店铺名称，填写 null，但请尽力查找
""",
