...
""",
    ...
    # 从列表页批量提取商品信息
    "extract_from_list": """
当前在小红书商品列表页面,请仔细识别并提取当前屏幕上所有可见商品卡片的信息。
...
对于每个商品卡片,请提取:
1. **商品标题**（卡片中部的标题文字）
2. **价格**（数字，如：99.9，不要包含 ¥ 符号）
3. **店铺名称**（卡片底部的店铺/卖家名称，这是最重要的信息！）
4. **位置索引**（第几个商品，从0开始计数）

**必须以标准JSON数组格式返回，不要有任何其他文字说明：**
```json
[
  {{
    "index": 0,
...
如果某个字段无法识别,请用 null 表示,但**店铺名称必须尽力提取**。
""",
...
""",
    ...
    # 提取商品信息
    "extract_info": """
当前在小红书商品详情页,请仔细识别并提取以下信息:

**关键信息（按优先级排序）：**
1. **店铺名称/卖家昵称**（通常在商品标题下方或价格附近，可能显示为"店铺"、"卖家"、"商家"等，这是最重要的信息！）
2. **商品标题**（页面顶部的商品名称）
3. **商品价格**（数字，如 199.0，不要包含 ¥ 符号）
4. **商品描述**（详细的商品介绍文字）
5. **页面所有文字**（OCR提取所有可见文字，用于后续内容匹配）

**必须以标准JSON格式返回，不要有任何其他解释性文字：**
```json
{{
  "shop_name": "店铺名称或卖家昵称",
  "title": "商品标题",
  ...
  "ocr_text": "页面上所有可见的文字内容..."
}}
```

**特别注意：**
- 店铺名称是盗版检测的核心依据，必须准确提取
- 如果页面有多个价格（原价、现价等），提取实际售价
- OCR文字要尽可能完整，包括标题、描述、评论等所有文本
- 所有字段都很重要，如果某个字段无法识别，请用 null 表示
